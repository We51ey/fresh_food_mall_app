<template>
	<view class="content">
		<view class="text-area">
			<van-field placeholder="请输入您的手机号码" border="false" title-width="5px" @change="phone=$event.detail"/>
			<van-field right-icon="eye-o" placeholder="请输入密码" border="false" password title-width="5px" @change="password=$event.detail"/>
			<van-field right-icon="eye-o" placeholder="请再次输入密码" border="false" password title-width="5px" @change="password2=$event.detail" />
			<van-cell-group>
		...
		methods: {
			...
			doConfirm() {
				...
				if (!reg.test(this.phone)) {
					...
					if (this.phone != getStorageSync('bind_phone')) {
						console.log('手机号错误')
						Toast('手机号错误');
					} else {
